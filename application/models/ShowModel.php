...
class ShowModel extends CI_Model{
	...
	public function searchRecentByUid($u_id,$beforeDay = 0)
	{
		$date = date('Y-m-d 00:00:00');
		$date = date('Y-m-d 00:00:00',strtotime("$date - $beforeDay day"));
		$rs = $this->db->query("SELECT `shows`.`s_id`,e_id,se_id,s_name,e_num,e_time 
			FROM episode 
			LEFT JOIN shows ON `episode`.`s_id` = `shows`.`s_id` 
			LEFT JOIN subscribe ON `shows`.`s_id` = `subscribe`.`s_id` 
			WHERE `subscribe`.`u_id` = {$u_id} AND `episode`.`e_time` >'{$date}'
	...
	{
		$rs = $this->db->query("SELECT `shows`.`s_id`,s_name,s_sibox_image,status,channel,update_time
			FROM shows 
			LEFT JOIN subscribe ON `shows`.`s_id` = `subscribe`.`s_id` 
